    ...
    while eps>tol && j<n_times %set some tolerance and do while here
        ...
        for i=1:N_monte_carlo
            ...
            if strcmp(method,'unif')
                [T(i,:), log_density(i)] = generate_uniform_times_sthp(data,p,bin_width);  
                weights_raw(i) = -complete_data_log_likelihood_sthp(T(i,:),baseline_est,excitation_est,decay_est,end_time)-(log_density(i));
            
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            % Sequential method %
            elseif strcmp(method,'seq')
                [T(i,:), ~, log_density(i)] = disc_time_sthp_grid(data, 1, [params(j,1),params(j,2),params(j,3)], bin_width, 1); 
                %Compute the Importance Sampling weights 
                weights_raw_seq(i) = exp((complete_data_log_likelihood_sthp(T(i,:),params(j,1),params(j,2),params(j,3),end_time)-log_density(i))); 
...
%              return
        ...
        if strcmp(method,'unif')
            ...
            weights_raw_rescaled = exp(weights_raw-min(weights_raw));
            weights_raw_rescaled = weights_raw_rescaled./sum(weights_raw_rescaled);
            
        elseif strcmp(method,'seq')
            weights_raw_rescaled = weights_raw_seq./sum(weights_raw_seq);
        end
        ...
        weights1_time=tic; [params(j+1,:), fval1(j),~,~,~,grad,hessian] = fmincon(myfun,zhat,[],[],[],[],[],[],[],options1); w1_time = toc(weights1_time); 
%       disp(w1_time)
        
        grads = [grads;grad];
        hessians = [hessians;hessian];
        fprintf('Current estimates: %i \n', params(j+1,:));
