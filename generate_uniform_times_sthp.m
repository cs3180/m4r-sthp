function [times_cell, log_density] = generate_uniform_times_sthp(data, p, bin_width)
...
log_density = sum(sum(log(factorial(data)))) + -sum(data)*log(bin_width); % double check add -sum(data)*log(bin_width) - should do by above eqn  
